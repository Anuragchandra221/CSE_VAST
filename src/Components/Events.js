...
function Events(props) {
  return (
    <div className='d-flex event justify-content-center hidden my-4 my-lg-4'>
        <div>
            <Link to={`/${props.link}`}><img className='event-img' src={props.src}  /></Link>
        </div>
        <div className='d-flex flex-column mx-3 mx-lg-0 ml-lg-4 mt-3 mt-lg-0 align-items-start'>
            ...
            <Link to={`/${props.link}`}><button className='event-btn px-3 py-2'>Explore <FontAwesomeIcon icon={faArrowTrendUp} /></button></Link>
        </div>
    </div>
  )
}
