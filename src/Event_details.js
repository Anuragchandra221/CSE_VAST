import dsd from "./Assets/Images/dsd.png"
import c2c from "./Assets/Images/c2c.png"
import udzn from "./Assets/Images/udzn.png"
import matrix from "./Assets/Images/matrix.png"
import animaze from "./Assets/Images/animaze.png"
import infinity from "./Assets/Images/infinity.png"
import photobooth from "./Assets/Images/photobooth.png"
import maze from "./Assets/Images/maze.png"
import respawn from "./Assets/Images/respawn.png"
import fifa from "./Assets/Images/fifa.png" 
import workshop from "./Assets/Images/workshop.png"

import codex from "./Assets/Images/CodeX Logo.png"
import dbug from "./Assets/Images/D_bug.png"
import darkcode from "./Assets/Images/DarkCode.png"
import refix from "./Assets/Images/ReFix.png"
import typrace from "./Assets/Images/TypRace.png"
import quizzy from "./Assets/Images/qwizzy.png"

import matrixL from "./Assets/Images/Events/matrix.jpeg"
import infinityL from "./Assets/Images/Events/infinity.jpeg"
import animazeL from "./Assets/Images/Events/animaze.jpeg"
import c2cL from "./Assets/Images/Events/c2c.jpeg"
import dsdL from "./Assets/Images/Events/dsd.jpeg"
import photoboothL from "./Assets/Images/Events/photobooth.jpeg"
import udznL from "./Assets/Images/Events/udzn.jpeg"
import mazeL from "./Assets/Images/Events/maze.jpeg"
import workshopL from "./Assets/Images/Events/workshop.jpg"
import fifaL from "./Assets/Images/Events/fifa.png"
import respawnL from "./Assets/Images/Events/respawn.jpg"

const cse = {
    "main": [
        {
            "title": "Respawn - E-Sports",
            "desc": "Valorent Esports Tournament.",
            "day": "Feb 29 & Mar 1",
            "time": "10AM Onwards",
            "fee": "500",
            "venue": "Auditorium",
            "prize": "1,00,000",
            "faculty": "Remya P S, Thulasi Rajan K",
            "student": "Appu K (S8), Sanju C S (S6)",
            "link": "respawnesports",
            "img": respawn
        }
    ],
    "technical": [
        {
            "title": "Department Expo: Matrix",
            "desc": "Step into Matrix - where innovation meets illumination.",
            "venue": "NB 208",
            "faculty": "Remya KR, Sivadasan E T, Neethu CS",
            "student": "Nikhil Mukundan (S6)",
            "link": "matrix",
            "img": matrix
        },
        {
            "title": "DEV-SHOWDOWN",
            "desc": "Gear up for the ultimate clash of technical brilliance at DevShowdown – a dynamic team event that blends innovation with competition. ",
            "day": "Feb 29 & Mar 1",
            "time": "9:15 Onwards",
            "fee": "300",
            "venue": "Systems Lab, Project Lab, Programming Lab, NB 213, NB 212, NB 211",
            "prize": "35,000",
            "faculty": "Jensy Babu, Athulya Bhaskar",
            "student": "Anurag Chandra (S8), Febin Rajan (S8)",
            "link": "dsd",
            "img": dsd,
            "sub": [
                {
                    "title": "CODEX",
                    "desc": "In the realm of CodeX, brilliance prevails, and only the sharpest minds emerge victorious. Are you the coding maestro?",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "Systems Lab",
                    "faculty": "Mithu Varghese",
                    "student": "Soloman Varghese (S8)"
                },
                {
                    "title": "DARKCODE",
                    "desc": "Step into the mystery of DarkCode, where participants code blind, the code being typed remains concealed until the competition concludes.",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "Project Lab",
                    "faculty": "Reshma V M",
                    "student": "Febin Rajan (S8)"
                },
                {
                    "title": "REFIX",
                    "desc": "Unleash your hardware mastery by reassembling a simple PC Component within a given time frame! So let the reassembly begin!",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "NB 213",
                    "faculty": "Lalini K S",
                    "student": "Ignatius Nobel (S8)"
                },
                {
                    "title": "QWIZZY",
                    "desc": "Embark on a journey through three levels of mind-bending questions, from basics to brilliance in the ultimate tech quiz challenge.",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "NB 212",
                    "faculty": "Renjima, Mrithu",
                    "student": "Rohit C (S8)"
                },
                {
                    "title": "TYPRACE",
                    "desc": "Race against time in TypRace, where swift fingers battle for victory. Can your typing speed outpace the ticking clock? Gear up for the ultimate typing showdown! ",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "Systems Lab",
                    "faculty": "Soumya",
                    "student": "Gokul P (S8)"
                },
                {
                    "title": "D_BUG",
                    "desc": "Dive into the world of debugging, racing against the clock to untangle complex code and unveil hidden errors.",
                    "day": "Feb 29",
                    "time": "9:15 Onwards",
                    "venue": "Project Lab",
                    "faculty": "Thulasi Rajan K",
                    "student": "Anurag Chandra (S8)"
                }
            ]
        },
        {
            "title": "UDZN",
            "desc": "UI designing competition.",
            "day": "Prior to 29 Feb",
            "fee": "50",
            "venue": "Online Event",
            "prize": "3,000",
            "faculty": "Athulya Bhaskar",
            "student": "Issak (S8)",
            "link": "udzn",
            "img": udzn
        },
        {
            "title": "CANVAS 2 CODE",
            "desc": "Front-end development challenge.",
            "day": "Prior to 29 Feb",
            "fee": "50",
            "venue": "Online Event",
            "prize": "3,000",
            "faculty": "Remya Raj",
            "student": "Aswin M M (S8)",
            "link": "c2c",
            "img": c2c
        },
        {
            "title": "ANIMAZE",
            "desc": "2D Animation Competition.",
            "day": "Prior to 29 Feb",
            "fee": "50",
            "venue": "Online Event",
            "prize": "3,000",
            "faculty": "Mrithu",
            "student": "Jibin C Baby (S8)",
            "link": "animaze",
            "img": animaze
        },
        {
            "title": "Workshop",
            "desc": "Elevate your skills and join the tech revolution.",
            "day": "Feb 29",
            "time": "9:15 Onwards",
            "fee": "200",
            "venue": "Common computer lab",
            "faculty": "Neethu P R, Aishwarya",
            "student": "Ragitha G (S8)",
            "link": "workshop",
            "img": workshop
        },
    ],
    "non": [
        {
            "title": "WAKA WAKA: FIFA Tournament",
            "desc": "Channel your inner Messi, unleash your Ronaldo rage: It's Waka Waka , the FIFA showdown where virtual boots meet glory.",
            "day": "Feb 29 & Mar 1",
            "time": "9:15 Onwards",
            "fee": "100",
            "prize": "12,000",
            "venue": "NB 215",
            "faculty": "Delmy",
            "student": "Abhinav M (S4), Jeevanand P S (S8)",
            "link": "wakawaka",
            "img": fifa
        },
        {
            "title": "MAZE RUNNER",
            "desc": "Be the fastest one through Labyrinths and Obstacles.",
            "day": "Feb 29 & Mar 1",
            "time": "9:15 Onwards",
            "fee": "30",
            "prize": "3,000",
            "venue": "NB 203",
            "faculty": "Shameem",
            "student": "Abhishek C (S6)",
            "img": maze,
            "link": "maze"
        },
        {
            "title": "INFINITY FUN",
            "desc": "Play to win but enjoy the game.",
            "day": "Feb 29 & Mar 1",
            "time": "9:15 Onwards",
            "fee": "20 per game",
            "venue": "NB 210",
            "faculty": "Neethu George",
            "student": "Aswathy C B (S6)",
            "img": infinity,
            "link": "infinity"
        },
        {
            "title": "PHOTO BOOTH",
            "desc": "Big smiles, tiny booth, instant memories.",
            "day": "Feb 29 & Mar 1",
            "time": "9:15 Onwards",
            "fee": "10",
            "venue": "NB 216",
            "faculty": "Anupama",
            "student": "Anjali Krishna (S8)",
            "img": photobooth,
            "link": "photobooth"
        } 
    ]
}

const individual = {
    "respawn": {
        "title": "Respawn - E-Sports",
        "desc": "Valorent Esports Tournament.",
        "day": "Feb 29 & Mar 1",
        "time": "10AM Onwards",
        "fee": "500",
        "venue": "Auditorium",
        "prize": "1,00,000",
        "faculty": "Remya P S, Thulasi Rajan K",
        "student": "Appu K (S8), Sanju C S (S6)",
        "link": "respawnesports",
        "img": respawnL,
        "registration": "https://forms.gle/64kao6SABBPUwHmP9",
        "rule": "https://docs.google.com/document/d/1SkXle9D5OfRC04Oj7iMXgY7r6eIYz_Bfk8E2Ad4O5oI/edit",
        "contact": [
            "Appu K - +91 9495428169"
        ],
    },
    "matrix": {
        "title": "Department Expo: Matrix",
        "desc": "Welcome to Matrix, a world where the fusion of technology and aesthetics creates a truly pleasing and unforgettable experience.Explore a future where holograms, augmented reality, and virtual reality now enhanced by AI converge, reshaping the limits of possibility. Engage with mind-bending experiences, interactive demos, and a sneak peek into the cutting edge of computer science.But that's not all – immerse yourself in a lighting project designed to enhance user experience and create a truly pleasing atmosphere.",
        "venue": "NB 208",
        "faculty": "Remya KR, Sivadasan E T, Neethu CS",
        "student": "Nikhil Mukundan (S6)",
        "link": "matrix",
        "img": matrixL,
        "contact": [
            "Nikhil Mukundan - +91 7012240531"
        ],
    },
    "dsd": {
        "title": "DEV-SHOWDOWN",
        "desc": "Gear up for the ultimate clash of technical brilliance at DevShowdown – a dynamic team event that blends innovation with competition. ",
        "day": "Feb 29 & Mar 1",
        "time": "9:15 Onwards",
        "fee": "300",
        "venue": "Systems Lab, Project Lab, Programming Lab, NB 213, NB 212, NB 211",
        "prize": "35,000",
        "faculty": "Jensy Babu, Athulya Bhaskar",
        "student": "Anurag Chandra (S8), Febin Rajan (S8)",
        "link": "dsd",
        "img": dsdL,
        "registration": "https://forms.gle/Maq1Sdt4YUqqsE5i6",
        "contact": [
            "Anurag Chandra - +91 7012597830"
        ],
        "guidelines": "Event date - Feb 29 & Mar 1.Each team should contain 3 members.Each member has to participate in two sub events.There is a registration fee of Rs 300.The winners will be awarded from a prize pool worth 35k.A total of 5 teams with the most cumulative individual points will advance to the next day.Top 3 will be awarded the prize money based on the completion of the treasure hunt.Participation certificates will be provided",
        "sub": [
            {
                "title": "CODEX",
                "desc": "In the realm of CodeX, brilliance prevails, and only the sharpest minds emerge victorious. Are you the coding maestro?",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "Systems Lab",
                "faculty": "Mithu Varghese",
                "student": "Soloman Varghese (S8)",
                "img": codex
            },
            {
                "title": "DARKCODE",
                "desc": "Step into the mystery of DarkCode, where participants code blind, the code being typed remains concealed until the competition concludes.",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "Project Lab",
                "faculty": "Reshma V M",
                "student": "Febin Rajan (S8)",
                "img": darkcode
            },
            {
                "title": "REFIX",
                "desc": "Unleash your hardware mastery by reassembling a simple PC Component within a given time frame! So let the reassembly begin!",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "NB 213",
                "faculty": "Lalini K S",
                "student": "Ignatius Nobel (S8)",
                "img": refix
            },
            {
                "title": "QWIZZY",
                "desc": "Embark on a journey through three levels of mind-bending questions, from basics to brilliance in the ultimate tech quiz challenge.",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "NB 212",
                "faculty": "Renjima, Mrithu",
                "student": "Rohit C (S8)",
                "img": quizzy
            },
            {
                "title": "TYPRACE",
                "desc": "Race against time in TypRace, where swift fingers battle for victory. Can your typing speed outpace the ticking clock? Gear up for the ultimate typing showdown! ",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "Systems Lab",
                "faculty": "Soumya",
                "student": "Gokul P (S8)",
                "img": typrace 
            },
            {
                "title": "D_BUG",
                "desc": "Dive into the world of debugging, racing against the clock to untangle complex code and unveil hidden errors.",
                "day": "Feb 29",
                "time": "9:15 Onwards",
                "venue": "Project Lab",
                "faculty": "Thulasi Rajan K",
                "student": "Anurag Chandra (S8)",
                "img": dbug 
            }
        ]
    },
    "udzn": {
        "title": "UDZN",
        "desc": "Dive into digital art's playground and turn pixelated paths to mind-bending reality. UDZN is a 48 hour design sprint where you'll battle against the clock to design a website so stunning, it'll break the internet. Unleash your creativity and walk away with glory and claim the throne as the digital design champion. ",
        "day": "Prior to 29 Feb",
        "fee": "50",
        "venue": "Online Event",
        "guidelines": "Challenge to create usable, aesthetically pleasing website design where form meets function flawlessly.Participants can craft their design with any fonts, colors, pictures, or elements that they feel are best suited to the task.The registration closes on 22/02/2024.The topic and form to upload entries will be put up on 23/02/2024.The participant must submit a Two-page Design of the topic given before the deadline for valid participation.Design source file can be uploaded in the provided form as [.ai, .fig or link].There is a registration fee of ₹50.Winners will be awarded from a prize pool of ₹3000.The designs will be judged by a panel, and the decision of the panel will befinal. The organizers reserve the right to use their discretion on the modification and enforcement of rules.",
        "prize": "3,000",
        "faculty": "Athulya Bhaskar",
        "student": "Issak (S8)",
        "link": "udzn",
        "img": udznL,
        "registration": "https://forms.gle/iVxZKPkVVjgF3jRJ9",
        "contact": [
            "Issak Thomas - +91 7907420280"
        ],
    },
    "c2c": {
        "title": "CANVAS 2 CODE",
        "desc": "Canvas2Code is a single-player event where participants are provided with the design of a website on Figma. They have to replicate the design onto a working website using their coding skills within the given time period. Against the relentless tick of the clock, contenders battle it out, pushing their coding skills to the limit in a fierce race to demonstrate precision, innovation, and speed.",
        "day": "Prior to 29 Feb",
        "fee": "50",
        "venue": "Online Event",
        "guidelines": "Participants are free to use any web framework or library.Registration closes on February 22 at 11:50 PM.The topic and form to upload entries will be put up on February 23 at 6:00 PMLink to GitHub repo containing source code.Link to hosted website.Submission deadline: February 25 at 6:00 PMThere is a registration fee of ₹50.The creations will be judged by a panel on the basis of adherence to design accessibility, responsiveness as well as other factors.The organizers reserve the right to use their discretion on the modification and enforcement of rules.",
        "prize": "3,000",
        "faculty": "Remya Raj",
        "student": "Aswin M M (S8)",
        "link": "c2c",
        "img": c2cL,
        "registration": "https://forms.gle/qoJSAjvEctc2jWz5A",
        "contact": [
            "Aswin M M - +91 7907048559"
        ],
    },
    "animaze": {
        "title": "ANIMAZE",
        "desc": "Are you ready to animate your dreams? Prepare to embark on an animation expedition at Animaze! Animaze is a 48 hour event where you can master cutting-edge tools, and push the boundaries of your imagination. Craft an animation frame by frame within allotted time that will captivate audiences, defy expectations, and etch your name in the annals of animation history! ",
        "day": "Prior to 29 Feb",
        "fee": "50",
        "venue": "Online Event",
        "guidelines": "Create a flawless animation using any animation technique, software, or tools that ignite your vision.The registration closes on 22/02/2024.The topic and form to upload entries will be put up on 23/02/2024.The participant must submit a frame by frame 2D animated video of the topic given before the deadline for valid participation.The animated video of any duration should not exceed one minute.Output can be uploaded in the provided form as any suitable video format.There is a registration fee of ₹50.Winners will be awarded from a prize pool of ₹3000.The animations will be judged by a panel, and the decision of the panel will befinal.The organizers reserve the right to use their discretion on the modificationand enforcement of rules.",
        "prize": "3,000",
        "faculty": "Mrithu",
        "student": "Jibin C Baby (S8)",
        "link": "animaze",
        "img": animazeL,
        "registration": "https://forms.gle/g29W5Vji1qP3MW4A8",
        "contact": [
            "Jibin C Baby - +91 8921275707"
        ],
    },
    "workshop": {
        "title": "Workshop",
        "desc": "Elevate your skills and join the tech revolution.",
        "day": "Feb 29",
        "time": "9:15 Onwards",
        "fee": "200",
        "venue": "Common computer lab",
        "faculty": "Neethu P R, Aishwarya",
        "student": "Ragitha G (S8)",
        "link": "workshop",
        "img": workshopL,
        "contact": [
            "Ragitha G - +91 7736764328"
        ],
    },
    "fifa": {
        "title": "WAKA WAKA: FIFA Tournament",
        "desc": "Channel your inner Messi, unleash your Ronaldo rage: It's Waka Waka , the FIFA showdown where virtual boots meet glory.",
        "day": "Feb 29 & Mar 1",
        "time": "9:15 Onwards",
        "fee": "100",
        "prize": "12,000",
        "venue": "NB 215",
        "faculty": "Delmy",
        "student": "Abhinav M (S4), Jeevanand P S (S8)",
        "link": "wakawaka",
        "img": fifaL,
        "registration": "https://forms.gle/9dzrVXYhKD7g4e2B6",
        "contact": [
            "Abhinav M - +91 9645320295",
            "Jeevanandh P S - +91 7592093835"
        ],
    },
    "maze": {
        "title": "MAZE RUNNER",
        "desc": "Embark on a heart-pounding journey through the maze of suspense and challenge. the maze runner event awaits, where every twist and turn tests the courage and wit of participants, leaving them on the edge of thrill.",
        "day": "Feb 29 & Mar 1",
        "time": "9:15 Onwards",
        "fee": "30",
        "prize": "3,000",
        "guidelines": "Follow the instructions provided by the volunteers present in the venu.If you get lost in the maze just yell “HELP”.Works on a First come First serve basis.",
        "venue": "NB 203",
        "faculty": "Shameem",
        "student": "Abhishek C (S6)",
        "link": "maze",
        "img": mazeL,
        "contact": [
            "Abhishek C - +91 9562823015"
        ],
    },
    "infinity": {
        "title": "INFINITY FUN",
        "desc": "Play to win but enjoy the game.",
        "day": "Feb 29 & Mar 1",
        "time": "9:15 Onwards",
        "fee": "20 per game",
        "venue": "NB 210",
        "faculty": "Neethu George",
        "student": "Aswathy C B (S6)",
        "link": "infinity",
        "img": infinityL,
        "contact": [
            "Aswathy C B - +91 6282579676"
        ],
    },
    "photo": {
        "title": "PHOTO BOOTH",
        "desc": "Big smiles, tiny booth, instant memories.",
        "day": "Feb 29 & Mar 1",
        "time": "9:15 Onwards",
        "fee": "10",
        "venue": "NB 216",
        "faculty": "Anupama",
        "student": "Athulia T Rajesh (S8)",
        "link": "photobooth",
        "img": photoboothL,
        "contact": [
            "Athulia T Rajesh - +91 8921331500"
        ], 
    }

}

const team = {
    "tech": {
        "captain": [{
            "name": "Shanavas R S",
            "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707979928/cse%20team/91_75929_94362_20200617_214229_eeol5f.jpg"
        }],
        "vice": [{
            "name": "Joel Joseph",
            "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707979711/cse%20team/WhatsApp_Image_2024-02-15_at_12.17.01_PM_udkr1i.jpg"
        }]
    },
    "media": {
        "title": "Media",
        "main": [
            {
                "name": "Abhinav A A",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707937130/Abhinav_A_A_-_maincoordinator_as7eee.jpg"
            }
        ],
        "sub": [
            {
                "name": "Swarag Anil",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708062061/cse%20team/Swarag_skwq4v.jpg"
            }
        ],
        "design":[
            {
                "name": "Suryajit Sudheeran",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707976965/cse%20team/Suryajit_m8kalw.jpg"
            }
        ],
        "designs": [
            {
                "name": "Joel Joseph",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707979711/cse%20team/WhatsApp_Image_2024-02-15_at_12.17.01_PM_udkr1i.jpg"
            }
        ],
        "vfx": [
            {
                "name": "Rohan V R",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059233/cse%20team/Rohan_VR_bnsyq6.jpg"
            }
        ]
    },
    "respawn": {
        "title": "Respawn",
        "main": [
            {
                "name": "Appu K",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059755/cse%20team/Appu_K_-_Main_Coordinator_zisw2l.jpg"
            },
            {
                "name": "Sanju C S",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708061074/cse%20team/Sanju_main_coordinator_ml4vwt.jpg"
            }
        ],
        "sub": [
            {
                "name": "Athul Shaju",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934747/cse%20team/athul_shaju-sub_coordinator_oo7oh5.jpg"
            }
        ]
    },
    "marketing": {
        "head": [
            {
                "name": "Neha Baju",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059923/cse%20team/Neha_Baju_ycuucg.jpg"
            }
        ],
        "heads": [
            {
                "name": "V S Swetha",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708061208/cse%20team/IMG_20240216_105148_aasqpu.jpg"
            }
        ],
        "member": [
            {
                "name": "Gopika Babu",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708061844/cse%20team/Gopika_Babu_dnnzjg.jpg"
            },
            {
                "name": "Devapriya Rajan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059930/cse%20team/IMG_1946_i4expo.jpg"
            },
            {
                "name": "Sivakami Prasanth",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059947/cse%20team/Sivakami_Prasanth_dnbgk1.jpg"
            },
            {
                "name": "Asna Haris",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708060358/cse%20team/Asna_Haris_as3luc.jpg"
            },
            {
                "name": "Namitha P Menon",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708061404/cse%20team/Namitha_P_Menon_rlfjcx.jpg"
            },
            {
                "name": "Ragitha G",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934726/cse%20team/RAGITHA_G_Main_Coordinator_wak29j.jpg"
            }
        ]
    },
    "dsd": {
        "title": "DevShowdown",
        "main": [
            {
                "name": "Anurag Chandra",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934748/cse%20team/20221217165025__MG_7780_1_gombe2.jpg"
            },
            {
                "name": "Febin Rajan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934739/cse%20team/FEBIN_RAJAN-MAIN_COORDINATOR_jyrshk.jpg"
            }
        ],
        "sub": [
            {
                "name": "R Akash",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934738/cse%20team/R.Akash_sub_coordinator_lbto8v.jpg"
            }
        ]
    },
    "codex": {
        "title": "Codex",
        "main": [
            {
                "name": "Solomon Varghese",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708058914/cse%20team/Soloman_Varghese_Main_Coordinator_rwlxty.jpg"
            }
        ],
        "sub": [
            {
                "name": "R Akash",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934738/cse%20team/R.Akash_sub_coordinator_lbto8v.jpg"
            }
        ]
    },
    "dbug": {
        "title": "D_bug",
        "main": [
            {
                "name": "Anurag Chandra",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934748/cse%20team/20221217165025__MG_7780_1_gombe2.jpg"
            }
        ],
        "sub": [
            {
                "name": "Athmanand V J",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934745/cse%20team/ATHMANAND_V_J_SUB_COORDINATE_irk4ru.jpg"
            }
        ]
    },
    "darkcode": {
        "title": "DarkCode",
        "main": [
            {
                "name": "Febin Rajan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934739/cse%20team/FEBIN_RAJAN-MAIN_COORDINATOR_jyrshk.jpg"
            }
        ],
        "sub": [
            {
                "name": "A Ardra Nair",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707978012/cse%20team/A_Ardra_Nair_Sub-Coordinator_1_ruhbfh.jpg"
            }
        ]
    },
    "quiz": {
        "title": "Qwizzy",
        "main": [
            {
                "name": "Rohit C",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934734/cse%20team/Rohit_C_-_Main_Coordinator_u7segj.jpg"
            }
        ],
        "sub": [
            {
                "name": "Angel Joy",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934738/cse%20team/Angel_Joy_Sub_Coordinator_eju1hd.jpg"
            }
        ]
    },
    "refix": {
        "title": "ReFix",
        "main": [
            {
                "name": "Ignatius Nobel",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934733/cse%20team/Ignatius_Nobel_Chittilappilly-_Main_Coordinator_oap7k1.jpg"
            },
            {
                "name": "Abhiram A S",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707976491/cse%20team/Abhiram_A_S_-_Main_Coordinator_phpprh.jpg"
            }
        ],
        "sub": [
            {
                "name": "Athul Paramel",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934733/cse%20team/Adhul_Paramel_Sathish-_Sub_coordinator_ci9ffk.jpg"
            }
        ]
    },
    "typrace": {
        "title": "Typerace",
        "main": [
            {
                "name": "Gokul P",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707977747/cse%20team/Gokul_P_-_Main_Coordinator_1_eow0p5.jpg"
            }
        ],
        "sub": [
            {
                "name": "Rasiya Sidharthan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708064657/cse%20team/Rasiya_Sidhartha_C_S_-_sub_coordinator_1_xvkpay.jpg"
            }
        ]
    },
    "animaze": {
        "title": "Animaze",
        "main": [
            {
                "name": "Jibin C Baby",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934741/cse%20team/Jibin_C_Baby_-_main_coordinator_p6mjky.jpg"
            }
        ],
        "sub": [
            {
                "name": "Issak Thomas",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708062277/cse%20team/WhatsApp_Image_2024-02-16_at_11.06.23_AM_cehq1v.jpg"
            }
        ]
    },
    "c2c": {
        "title": "Code 2 Canvas",
        "main": [
            {
                "name": "Aswin MM",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934741/cse%20team/Aswin_MM_-_Main_coordinator_tf0r1o.jpg"
            }
        ],
        "sub": [
            {
                "name": "Najil Arfak",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707976586/cse%20team/Najil_Arfak_NN_-_Sub_coordinator_goffq1.jpg"
            }
        ]
    },
    "matrix": {
        "title": "Matrix",
        "main": [
            {
                "name": "Nikhil Mukundan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707978032/cse%20team/NIKHIL_MUKUNDAN_-_MAIN_COORDINATOR_1_bgz8sq.jpg"
            }
        ],
        "sub": [
            {
                "name": "H Athulkrishnan",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934734/cse%20team/H_ATHULKRISHNAN_-_SUB_COORDINATOR_aasdmc.jpg"
            }
        ]
    },
    "maze": {
        "title": "Maze runner",
        "main": [
            {
                "name": "Abhishek C",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934729/cse%20team/ABHISHEK_C_-_Main_Coordinator_vx4uhk.jpg"
            }
        ],
        "sub": [
            {
                "name": "Mohamed Riyas",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707976259/cse%20team/MOHAMEDRIYAS_mukhfw.jpg"
            }
        ]
    },
    "photo": {
        "title": "Photobooth",
        "main": [
            {
                "name": "Athulia T Rajesh",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934728/cse%20team/Athulia_T_Rajesh-_Main_Coordinator_bullrm.jpg"
            }
        ],
        "sub": [
            {
                "name": "Deva Priya",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934728/cse%20team/Deva_Priya-_Sub_Coordinator_dzf1aq.jpg"
            }
        ]
    },
    "udzn": {
        "title": "UDZN",
        "main": [
            {
                "name": "Issak Thomas",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708062277/cse%20team/WhatsApp_Image_2024-02-16_at_11.06.23_AM_cehq1v.jpg"
            }
        ],
        "sub": [
            {
                "name": "Aswin P Prakash",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934726/cse%20team/Aswin_P_Prakash_-_Sub_Coordinator_yoyryu.jpg"
            }
        ]
    },
    "workshop": {
        "title": "Workshop",
        "main": [
            {
                "name": "Ragitha G",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707934726/cse%20team/RAGITHA_G_Main_Coordinator_wak29j.jpg"
            }
        ],
        "sub": [
            {
                "name": "Divya Devaraj",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1707937854/Divya_Devaraj_sub_coordinator_luykjz.jpg"
            },
            {
                "name": "Gouri Rajeev",
                "url": "https://res.cloudinary.com/dyp9x1eso/image/upload/v1708059637/cse%20team/Gouri_Rajeev_Sub_coordinator_crgafw.jpg"
            }
        ]
    },
}

export {cse, individual, team}